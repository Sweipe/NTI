class Navmesh {
	...
	static Compute(gameobjects) {
		...
		for(const gameobject of gameobjects) {
			...
			let outerCorners = [
				new Vector(center.x - w,center.y - h+1),
				new Vector(center.x + w,center.y - h),
				new Vector(center.x + w,center.y + h-1),
				new Vector(center.x - w,center.y + h),
			]
		...
		let posA, posB, nLine, point, is_hit
		for(a = 0; a < corners.length; a++) {
			posA = corners[a]
			for(b = a+1; b < corners.length; b++) {
				posB = corners[b]
				...
				// for(const line of lines) {
					// point = Vector.Intersects(nLine.A,nLine.B,line.A,line.B)
					// if(point.hit) {
						// is_hit = true
						// break
					// }
				// }
				if(!is_hit) lines.push(nLine)
			}
		}
			...
			for(b = a-1; b>=0; b--) {
				...
				if(point.hit) {
					if(blue.A.Distance(blue.B)<red.A.Distance(red.B)) {
						...
						Navmesh.intersects.push(point.point)
					}
					else {
					...
					if(!nodeA.friends.includes(nodeB)) {
						...
						if(not_hit) {
							...
							for(const edge of edges) {
								point = Vector.Intersects(nodeA.pos,nodeB.pos,edge.A,edge.B)
								if(point.hit) {
									not_hit = false
									break
								}
							...
							if(not_hit) {
								Navmesh.red.push({A:nodeA.pos,B:nodeB.pos})
								Navmesh.intersects.push(point.point)
							}
						}
					}
	...
	static r2 = 0
	static DrawLine() {
		if(Navmesh.red.length>Navmesh.r2) {
			let red = Navmesh.red[Navmesh.r2]
			Draw.Line(red.A,red.B, 'limegreen')
		}
	}
